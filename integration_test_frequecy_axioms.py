...
for fold in dataset:
    for name in dataset[fold]:
        dataset[fold][name] = dataset[fold][name]

        for i in range(len(dataset[fold][name])):
            dataset[fold][name][i] = dataset[fold][name][i]

axiom_list = [ChangeAxiom] # MinMaxAxiom, MaxAxiom, MinAxiom, ChangeAxiom, IntegralAxiom, RelativeChangeAxiom, FirstDiffAxiom, SecondDiffAxiom]

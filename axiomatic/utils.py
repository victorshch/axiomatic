...
def time_series_embedding(series, left_neighbourhood, right_neighbourhood):
    ...
    a neighborhood and the row position corresponds to central point of the neighborhood in the original series. Rows where the neighborhood exceeds series limits contain NaN values. The part where neighborhood doesn't
    ...
    In [94]: time_series_embedding(series, 1, 2)
    Out[94]: 
        -1    0    1    2
    0  NaN  1.0  2.0  3.0
    1  1.0  2.0  3.0  4.0
    ...
    4  4.0  5.0  NaN  NaN
    """
    ts_embedding_list = [(-shift_value, series.shift(shift_value)) for shift_value in xrange(left_neighbourhood,  -right_neighbourhood - 1, -1)]
